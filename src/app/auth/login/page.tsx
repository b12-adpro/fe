...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    if (registered === "true") {
      ...
      if (registeredEmail) {
        setEmail(decodeURIComponent(registeredEmail)) // Pre-fill email
      }
    }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    await login(email, password) // Kirim email dan password
  }
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-xl shadow-lg">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="rounded-md shadow-sm -space-y-px">
            <div>
              <label htmlFor="email-address" className="sr-only">Email address</label> {/* Ganti username menjadi email */}
