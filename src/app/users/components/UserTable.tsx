...
export default function UserTable() {
  ...
  const handleBlock = async (id: string, status: boolean) => {
    try {
      await fetch(`http://localhost:8080/admin/users/${id}/block?status=${status}`, { method: 'POST' });
      fetchUsers();
    } catch (error) {
  ...
  const handleViewDonations = async (donaturId: string, userName: string) => {
    try {
      ...
      const res = await fetch(`http://localhost:8080/admin/donation-history/donatur/${donaturId}`, { method: 'GET' });
      const data = await res.json();
